function P = CoeficientesDSF(T,V,N)
...
c_ns= -(exp(j*pi*(1-n))-1)./((1-n).*4*pi)-(exp(-j*pi*(1+n))-1)./((1+n).*4*pi);
...
for iter = -N:1:N
    ...
    acumulador = acumulador + c_n(iter+N+1) .* exponencial;
     if iter==1
        acumulador2 = acumulador2 +(j*pi+(exp(-j*pi*(1+iter))-1)./(1+iter))./(-4*pi) .* exponencial;
    elseif iter==-1
    acumulador2 = acumulador2-(exp(j*pi*(1-iter))-1)./((1-iter).*4*pi)+(j*pi)./(4*pi) .* exponencial;
    else
...
c_ns(N)= -(exp(1i*pi*(1-iter))-1)./((1-iter).*4*pi)+(1i*pi)./(4*pi);
c_ns(N+2)= (1i*pi+(exp(-1i*pi*(1+iter))-1)./(1+iter))./(-4*pi);
figure
stem(n,abs(c_ns))
xlabel('Numero de coeficiente [k]')
ylabel('|c_n|')
