...
%% Represento las señales recibidas
  ...
  grid on
  %% Representacion en frecuencia de señal original y multiplexada
  
  x_1_frec = fft(x_1, NFFT)/NFFT;
   figure
   subplot(2,1,1)
   plot(frec, 20*log10(abs(x_1_frec(1:lg))));
   title('Usuario 1 en frecuencia')
   hold on
   axis ([0 10 -60 10]);
   grid on
  ...
  figure
  x_2_frec = fft(x_2, NFFT)/NFFT;
   figure
   subplot(2,1,1)
   plot(frec, 20*log10(abs(x_2_frec(1:lg))));
   title('Usuario 2 en frecuencia')
   hold on
   axis ([0 10 -60 10]);
   grid on
